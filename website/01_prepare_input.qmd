...
The figure below shows the distribution of qValues for the peaks from each TF. Overall, the distributions for Klf4, Oct4, and Sox2 are similar, whereas the curve for Nanog is shifted to the right, indicating that the peaks called in the Nanog experiment generally have a better "quality".
...
As shown below, the number of peaks called in the experiments for Klf4 and Nanog is 2- and 4-times larger compared to Oct4 and Sox2 respectively. Given that the sequencing depth for Oct4 is the largest (approx. 250 M reads), the most likely reason for this difference is that Oct4 has fewer genomic bindings sites or that the antibody used in the experiment is less specific. In contrast, the total number of reads for Sox2 is the lowest (approx. 140 M), meaning the relatively small number of peaks could also be attributed to the low sequencing depth. 
